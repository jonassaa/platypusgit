...
- **`actions.ts`** — catalog. Every action: `{ id, title, category, scope,
  allowInInput?, run? }`. Global app behaviors (navigation via nav intents,
  palette open, repo ops via `features/repo/ops.ts`, pane traversal,
  cheat-sheet) are **default runners in the catalog**, not useEffect wiring.
  Runners return `false` to decline (nothing to do → key falls through).
...
| Action | Chord |
| --- | --- |
| Commit screen | ⌘K (also ⌘2) |
| Push | ⌘⇧K |
| Pull / update | ⌘T |
| Fetch | ⌘⇧T |
| Refresh (sync) | ⌘⌥Y |
| Diff viewer | ⌘D (also ⌘8) |
| History / log | ⌘9 (also ⌘3) |
| Palette | ⌘P / ⌘⇧A / double-Shift |
| Screens | ⌘1…⌘7, reflog ⌘⇧9, settings ⌘, |
