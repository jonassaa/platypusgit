...
- `src/design/context-menu.compare.test.tsx` — the menu entries + the mark pair.

---

### Task 1: Backend — `diff_ref_to_workdir`

- [x] `ref_compare.rs` first: a repo with a committed baseline, then a staged
...
- [x] Trait method on `GitBackend` with the doc from spec §C/§D1 (why
      `_with_index`, what `include_untracked` means, who else will call it).
- [x] `Libgit2Backend`: `revparse_single(revspec).and_then(peel_to_tree)` mapped
...
- [x] `CliBackend` stub.
- [x] `commands/diff.rs::diff_ref_to_workdir` — `ignore_whitespace` and
...
- [x] `ref_compare.rs`: build `main` and `feature` diverged by 2 and 3 commits.
...
- [x] `commits_between("main", "feature", 10)` → the 3 feature-only commits
...
- [x] `types.rs`: `AheadBehind { ahead, behind, merge_base }`, `serde` rename to
      camelCase like its neighbours.
- [x] Trait + `Libgit2Backend` (`graph_ahead_behind`, `merge_base` → `Option`) +
      `CliBackend` stubs.
- [x] `commands/commits.rs`: both commands, `limit: Option<usize>` defaulted like
      `get_log`.
- [x] Register in `lib.rs`; `cargo test`.

### Task 3: TS types + wrappers + nav intent

- [x] `lib/types.ts`: `AheadBehind`.
- [x] `lib/tauri.ts`: `diffRefToWorkdir`, `aheadBehind`, `commitsBetween`, each
      documented with the direction convention.
- [x] `useNavStore`: `{ kind: "ref-compare"; left: CompareSide; right: CompareSide }`.
...
- [x] `CompareSide`, `sideLabel`, `sideKey` (a stable string for effect deps),
...
- [x] `DEFAULT_LEFT` derivation from the branch list (`currentBranch(...) ?? "HEAD"`)
      as a pure function taking the branches array, so the screen has no fallback
      logic of its own.
- [x] `compareSides.test.ts` covering each of the above.

### Task 5: `useCompareStore`

- [x] State: `repoId`, `left`, `right`, `diffs`, `aheadBehind`, `aheadCommits`,
      `behindCommits`, `loading`, `error`, `marked`.
- [x] `open(left, right)` sets both sides, stamps `repoId` from
      `useRepoStore.getState().current`, clears results.
- [x] `refresh(contextLines, ignoreWhitespace)`: captures `repoId` first and
...
- [x] rev↔rev issues `aheadBehind` + two `commitsBetween` + `diffCommits`;
      rev↔workdir issues `diffRefToWorkdir(…, includeUntracked: true)` only and
      clears the commit state.
- [x] `setLeft` / `setRight` / `swap` / `mark(ref)` / `clearMark()`.
- [x] Errors are stored locally (`error`), never pushed into `useRepoStore` — a
...
- [x] Popover anchored to the chip, portal'd, closing on outside mousedown and
...
- [x] Sections: local branches, remote branches, tags, `Working tree` (right side
      only), and a free-form row that accepts whatever was typed as a revspec.
- [x] Rows are density-aware; the picker itself is chrome.
- [x] `data-testid`: `compare-side-left`, `compare-side-right`,
      `compare-side-option`.

### Task 7: `screens/Compare.tsx`

- [x] `DeepViewHeader crumbs={[compareHeader(left, right)]}`.
- [x] Bar (`data-testid="compare-bar"`): left chip · `→` · right chip · swap ·
...
- [x] rev↔rev body: a vertical split — top section holds two `PGPane`s side by
...
- [x] rev↔workdir body: `CommitDiffPanel` only, plus the dimmed
      "untracked files included" note on the right chip.
- [x] `syntaxSides`: `old = { kind: "rev", rev: left.rev }`,
...
- [x] Re-fetch effect keyed on `sideKey(left)`, `sideKey(right)`,
      `diffContextLines`, `ignoreWhitespace`, `repo.id` — primitives only, never
      the side objects, which are rebuilt each render.
- [x] Commit rows are a local row component (sha · summary · author · relative
...
- [x] `AppShell`: `"compare"` in `ScreenId`, in `DEEP_VIEWS`, in the screens map;
      `case "ref-compare": enterDeep("compare")` in the intent switch.
- [x] `DeepViewHeader` needs no change: a deep view is never an ORIGIN, so it
      never appears in `SCREEN_LABELS`.
- [x] `context-menu.tsx`: `compareMenuItems({ ref, isCurrent })` returning the
...
- [x] `commands.ts`: **Compare refs…** and **Compare with working tree…**, both
      `step(() => ({ kind: "pick", … branchItems({ … }) }))`.
- [x] `AppShell.screens.test.tsx` — no change after all: its sweep is explicitly
      "every screen reachable from the activity bar", and `compare` is a deep
      view. Covered by `Compare.test.tsx` instead.

### Task 9: Frontend tests + full verification

- [x] `Compare.test.tsx`: rev↔rev renders summary + both lists and calls
...
- [x] Small Conventional Commits on `feat/branch-compare`; push; draft PR
