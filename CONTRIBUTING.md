...
- Do not create empty or merge commits — neither says anything, and the squash quotes both into the body. Do not amend published commits.

## Branching & merge workflow

We use **feature branches + squash and merge**. The `main` ruleset requires
linear history and allows squash merges only, so your branch arrives as **one
commit per PR**, with no merge commits.

1. Branch off `main` — `feat/...`, `fix/...`, `chore/...`, `docs/...`. Never commit to `main` directly.
2. Build the change as a series of small, focused commits (Conventional Commits throughout). They are collapsed on merge, but **every commit message is quoted into the squash commit's body**, so write each one as prose someone will read in `git log`.
3. If the branch needs updating, **rebase onto `main`** (`git rebase main`), never merge `main` in. Force-push with `--force-with-lease` after a rebase. A squash would collapse a merge commit harmlessly, but its message still lands in the body saying nothing.
4. Before marking the PR ready, tidy the branch's history with `git rebase -i main` — fold fixups, drop `wip` commits, reword subjects. That list is exactly what reviewers read and what the squash commit's body will quote.
5. PRs are integrated with GitHub's **Squash and merge**. Rebase and merge-commit are blocked. The squash title is the lone commit's subject for a one-commit PR and the PR title otherwise, with `(#N)` appended automatically — so let GitHub write it rather than overriding with `gh pr merge --subject`, which drops that backlink.
6. A branch that is only *behind* `main` still merges — GitHub squashes onto the current tip, which keeps history linear on its own. No rebase-before-merge dance: merge once GitHub reports the PR mergeable. Rebase locally only if GitHub reports conflicts, or you want CI to run against something that just landed on `main`.
