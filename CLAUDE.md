...
Do not create empty / merge commits — neither says anything, and the squash
quotes both into the body. A branch's commits are collapsed into ONE commit on
`main`, but every message is quoted into that commit's body, so no `wip` /
`fixup!` subjects should survive to the PR. Do not amend published commits
without asking.
...
- Work as a series of small, focused commits on the feature branch (Conventional Commits throughout). They do NOT land individually — the merge squashes them — but each message is quoted into the squash body, so write every one as public prose.
- When the branch does need updating, **rebase onto `main`**, not merge `main` in — no merge commits on the branch. A squash collapses one harmlessly, but its message still lands in the body as a `* Merge branch 'main'…` bullet that says nothing.
- Integrate via **squash and merge** (`gh pr merge <N> --squash`) — the `main` ruleset (id `18319179`) gates the method through `allowed_merge_methods`, which currently lists ONLY `squash`. **Verify it every time** (`gh api repos/:owner/:repo/rulesets/18319179`): the method has flipped before and this file lagged it by hours, so when the two disagree believe the ruleset and say so. Alongside it: `required_linear_history`, `non_fast_forward`, no deletion of `main`, `required_approving_review_count: 0`, and one required check, `e2e-linux`. **One commit lands on `main` per PR** — linear by construction.
- **What the squash commit says.** Title is `COMMIT_OR_PR_TITLE`: a one-commit PR keeps that commit's subject, a multi-commit PR takes the PR title — and GitHub appends `(#N)` either way. Body is `COMMIT_MESSAGES`: every branch commit's full message, bulleted. So still curate with `git rebase -i <pinned origin/main sha>` to fold fixups, drop `wip`, and reword subjects — not because those subjects reach `main`'s log any more, but because they reach that commit's body, and there is no second chance to clean it up.
- **Let GitHub title the squash.** `gh pr merge --subject` overrides the title and SILENTLY drops the `(#N)` backlink — measured on `9b6c06b`, the one commit in recent history missing it. Pass `--subject`/`--body` only to say something the branch commits do not, and spell the `(#N)` yourself when you do. The PR title and body still drive the PR page and issue auto-closing.
- **No rebase-before-merge requirement.** `required_linear_history` is satisfied by the squash itself (GitHub replays one commit onto the current `main` tip), and the required `e2e-linux` check is non-strict, so a branch that is merely *behind* `main` merges fine. **Merge as soon as GitHub reports the PR mergeable** (`gh pr view <N> --json mergeable,mergeStateStatus`). Rebase locally only when there's a reason: GitHub reports conflicts (`mergeable: CONFLICTING`), or your change interacts with something that landed on `main` since and you want CI to run against it.
