...
- **Inside a menu, `jsClickMenuItem` PRESSES first, in its own round trip, and that is load-bearing.** It used a bare `HTMLElement.click()` — a `click` with no preceding `mousedown`, which no mouse can produce — while menus dismiss on the press. Every submenu entry (Reset ▸ Soft/Mixed/Hard among them) was dead under a real mouse from the initial release to v0.8.0 with this suite green (#422). A helper that synthesizes an event sequence no user can produce is not testing the feature; if you add one, say which real sequence it stands in for.
